  ...
  methods: {
    async fetchSubscriber() {
      try {
        ...
        const response = await axios.get(`http://localhost/api/v1/subscribers/${params.id}`);
        const data = response.data;
        const fields = [];

        this.subscriber.fields.forEach(field => {
          const found = data.fields.find(subscriberField => subscriberField.id === field.id);
          if(found) {
            field.value = field.type == 'boolean' ? Boolean(parseInt(found.pivot.value)) : found.pivot.value;
          }
    ...
    async handleSubmit() {
      try {
        ...
        await axios.put(`http://localhost/api/v1/subscribers/${data.id}`, data);
